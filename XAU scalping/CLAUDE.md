...
### [PENDIENTE] Regla MER pág.28 — no implementada
Plan Técnico pág.28: el último alto/bajo m3 que fue **traspasado** durante la estructura tendencial anterior al ChOC debe ser superado con cuerpo ≥0.01% para que la entrada MER sea válida. Si no se supera ese nivel → el ChOC es válido para cambio de estructura pero NO para tomar MER. Nuestro código solo verifica que el cuerpo rompa `last_m3_high/low` con ≥0.01% — no verifica que ese nivel haya sido el "último traspasado en la estructura tendencial". Análisis adicional necesario antes de implementar (posible interacción con `prev_m3_high/low`).

### [PENDIENTE] Umbral inferior doji: posible exclusión de cuerpos <15%
Plan Técnico describe "vela envolvente doji" con cuerpo **cercano al 15%**, ambas mechas pronunciadas. Nuestro código exige `bpct >= 0.15`. Una vela con `bpct` entre 0.05–0.14 y ambas mechas pronunciadas podría calificar visualmente para Fabian pero nuestro código la rechaza. Hipótesis surgida en sesión 5-jul (ChOC bar ~09:15 que Fabian tomó como MER pero nuestro código no disparó por `entry_bull = false`). Pendiente confirmar con replay + tabla debug.
...
## Documentos de Fabian (guardados en `XAU scalping/docs/` desde 4-jul-2026)
- **Plan Operativo** (`docs/Plan_Operativo.pdf`, 1 pág — pendiente versión completa) — horario con notas DST; reglas de noticias: Forex Factory solo USD, feriados bancarios = no operable, alto impacto = ventana -10min/+3min (no abrir ni cerrar), medio impacto = ventana -3min/+3min (solo bloquea aperturas nuevas, una operación abierta se puede mantener), orador de impacto medio con hora programada = tratar como alto impacto, se puede entrar post-ventana al mismo precio si no se alcanzó el SL durante la ventana, 5 eventos de no-sostener-operación-abierta (Federal Funds Rate & Statement, NFP, CPI y/y, FOMC Minutes, Advance GDP q/q, todos USD); 3 escenarios de stop diario (1 TP / 1 SL+1TP / 2 SL)
- **Plan Técnico** (`docs/Plan_Tecnico.pdf`, 31 págs completas, leído íntegro 5-jul-2026) — la referencia más completa: estructura m3 (incl. variantes de fractal extendido), líneas punteadas (tendencial) vs continuas (reversión/ChOC), los 3 tipos de vela envolvente con sus % exactos, **Cuadrícula de Gann** = herramienta visual de TradingView para medir proporciones cuerpo/mecha (niveles 0/0.85/1 envolvente, 0/0.5/0.85/1 martillo, 0/0.15/0.85/1 doji, 0/0.25/0.5/0.75 indecisión/START), patrón START (morning/evening star), MEC (2 escenarios + 4 sub-tipos), MER (cambio de estructura + regla de "único nivel m3 opuesto" + "solo primer toque" pág.24), **MER pág.28: el último m3 tendencial anterior al ChOC debe ser superado con ≥0.01% — si no, NO hay entrada MER aunque sí haya ChOC** (pendiente implementar), concepto **Hedge Position** (netting nativo Pine ya lo maneja), SL (último m3 ± reducción 40% si >20.000 pips), **RR 1:0.9 confirmado explícito en la última página**
- **Apariencia del Indicador** (`docs/Apariencia_Indicador.pdf`, 2 págs) — spec visual de las etiquetas BUY/SELL, ejemplos reales de trades, confirmada = nuestro código ✅
...
- `sesion_vivo_2026-07-05/sesion_log.md` — lectura completa Plan Técnico 31 pág, análisis Gann, hipótesis doji `bpct<0.15`, hallazgo MER pág.28 sin implementar
